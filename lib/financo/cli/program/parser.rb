    ...
    class Program
      class Parser
        ...
        def initialize
          ...
          @op.on("--checking ACCOUNT_NAME", "bank checking account (default '#{DEFAULT_CHECKING}')")
          @op.on("-o", "--output OUTPUT", "journal output: filename or STDOUT (default: #{DEFAULT_OUTPUT})")
        ...
        def parse(argv)
          ...
          unless options[:help] || options[:version]
            raise ParserError.new("expected: '<username> <password>'.") if args.length != 2
          end
